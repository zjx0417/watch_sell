...
if(i==0){//如果滚动从头开始，再重新加上transtion
 if(to>i){ //如果要看当前位置右边的图片，是不会出问题的
   ulImgs.className="transition"
    ...
    //将ulImgs拉取到最左侧
       ...
       //定时器是为了将投梁换柱操作和加回transition属性的操作强行隔离开
       setTimeout(function(){
           moveTo(LICOUNT-1)
       },100)
       return
...
//调用两个按钮公开的移动方法，参数1表示移动到i+1的位置，相当于左移一个
move(1)
}
...
var as=document.querySelectorAll("#tab li")
    var x=10
    for(var a of as){
        a.onmouseover=function(e){
           ...
           var a=e.target;
           var ss=document.getElementsByClassName("active2")[0]				  
           if(ss!=null){
               ss.className=""
           }
           if(a.onmouseover){
    ...
    var ab=document.querySelectorAll("#tab2 li")
    var x=10
    for(var a of ab)
     a.onmouseover=function(e){
         ...
         var div=document.getElementById(id)
         var a=e.target
         var dd=document.getElementsByClassName("active3")[0]
         if(dd!=null){
            dd.className=""
         }
         if(a.onmouseover){
